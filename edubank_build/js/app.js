...
function colegios(){return layout('Colegios',`<div class="panel"><div class="paneltitle"><h2>Colegios registrados</h2><button class="primary smallbtn" onclick="addSchool()">+ Colegio</button></div>${schoolList.map(s=>`<div class="listrow"><div><b>${s.name}</b><small>${s.code} · ${s.city||'—'}</small></div><span class="status">${s.active?'Activo':'Inactivo'}</span></div>`).join('')||empty()}</div>`)}
async function usuarios(){let r=await window.eduBankSupabase.from('profiles').select('id,full_name,dni,role,school_id,active').order('full_name');let ps=r.data||[];return layout('Usuarios',`<div class="panel"><div class="paneltitle"><h2>Usuarios reales</h2><button class="primary smallbtn" onclick="addUser()">+ Usuario</button></div>${ps.map(p=>`<div class="listrow"><div><b>${p.full_name}</b><small>${p.role.toUpperCase()} · ${p.dni||'—'} · ${schoolName(p.school_id)}</small></div><span class="status">${p.active?'Activo':'Inactivo'}</span></div>`).join('')||empty()}</div>`)}
...
async function render(v){let out;if(me.role==='alumno')out=v==='inicio'?studentHome():v==='cuenta'?account():v==='movimientos'?movimientosStudent():v==='tienda'?store():loans();else out=v==='inicio'?adminHome():v==='alumnos'?renderAlumnos():v==='recompensas'?rewards():v==='aulas'?aulas():v==='docentes'?docentes():v==='colegios'?colegios():v==='usuarios'?await usuarios():v==='reportes'?reportes():movements();$('#content').innerHTML=out}
...
async function addSchool(){
 const name=prompt('Nombre del colegio:'); if(!name?.trim()) return;
 const code=prompt('Código del colegio:'); if(!code?.trim()) return;
 const city=prompt('Ciudad:','Lima');
 const {error}=await window.eduBankSupabase.from('schools').insert({name:name.trim(),code:code.trim(),city:city?.trim()||null,active:true});
 if(error)return alert('No se pudo crear el colegio: '+error.message);
 await loadData(); render('colegios'); alert('Colegio creado correctamente.');
}
async function addUser(){
 if(me.role!=='superadmin') return alert('Solo el SuperAdmin puede crear usuarios.');
 if(!schoolList.length) return alert('Primero crea un colegio.');
 const name=prompt('Nombre completo:'); if(!name?.trim()) return;
 const email=prompt('Correo de acceso:'); if(!email?.trim()) return;
 const password=prompt('Contraseña temporal (mínimo 6 caracteres):'); if(!password || password.length<6)return alert('La contraseña debe tener al menos 6 caracteres.');
 const role=prompt('Rol: superadmin, director, docente o alumno','alumno')?.trim().toLowerCase();
 if(!['superadmin','director','docente','alumno'].includes(role))return alert('Rol inválido.');
 const schoolId=role==='superadmin'?null:(prompt('ID del colegio (puedes copiarlo desde Supabase):',schoolList[0]?.id)||'').trim();
 const dni=prompt('DNI (opcional):')?.trim()||null;
 const {data,error}=await window.eduBankSupabase.functions.invoke('admin-create-user',{body:{email:email.trim(),password,full_name:name.trim(),role,school_id:schoolId||null,dni}});
 if(error)return alert('No se pudo crear el usuario: '+(error.message||error));
 await loadData(); render('usuarios'); alert('Usuario creado correctamente.');
}
