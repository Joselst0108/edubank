...
async function init(){
 ...
 if(error||!profile){await supabase.auth.signOut(); location.href='login.html'; return;}
 me=profile;
...
async function loadData(){
 ...
 let cq=supabase.from('classes').select('*, teacher:teacher_id(id,full_name,role)').order('name');
 if(me.role!=='superadmin') cq=cq.eq('school_id',sid);
 r=await cq; classes=r.data||[]; if(r.error) return fatal(r.error.message);
 let sq=supabase.from('students').select('*, profile:profile_id(id,full_name,dni,role), class:class_id(id,name)');
 ...
 let tq=supabase.from('profiles').select('*').eq('role','docente'); if(me.role!=='superadmin') tq=tq.eq('school_id',sid); r=await tq; teachers=r.data||[];
 let txq=supabase.from('transactions').select('*').order('created_at',{ascending:false}).limit(100);
...
async function buy(name,cost){if(!students[0])return;const {error}=await supabase.rpc('spend_educoins',{p_student:students[0].id,p_amount:cost,p_title:name,p_meta:'Tienda escolar'});if(error)return alert(error.message);await loadData();render('tienda');alert('Canje realizado en la cuenta real.')}
